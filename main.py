...
from src.othello_minimax import *
    ...
    while (run):
        ...
        if not check_canPlay and gameIsrunning:
            ...
            if (not board.canPlay(board.currentPlayer)):
                ...
                src.display.drawBar(WIN, board.currentPlayer, playerPlayTime)
                src.display.updateNbExplo(WIN, Nb_exploration[0])
        ...
        if Players[int(board.currentPlayer)] != False:
            ...
            #result = minimax(Node(board), Players[int(board.currentPlayer)].depth, True, board.currentPlayer)
